...
DescriptorBuilder::DescriptorBuilder(vk::Device &device) : _device(device) {}
...
vk::DescriptorSet DescriptorBuilder::build(vk::DescriptorSetLayout& layout) {
    ...
    auto pool = _device.createDescriptorPool(poolCI);
    ...
    layout = _device.createDescriptorSetLayout(
    ...
    auto sets = { layout };
    const auto descriptorSet = _device.allocateDescriptorSets(vk::DescriptorSetAllocateInfo()
...
void DescriptorBuilder::write_image(const u32 dstArrayElement, vk::ImageView image, vk::Sampler sampler, vk::ImageLayout layout, const vk::DescriptorType type) {
...
void DescriptorBuilder::update_set(const vk::DescriptorSet& set) {
    ...
    _device.updateDescriptorSets(writeCount, writes.data(), 0, nullptr);
}
